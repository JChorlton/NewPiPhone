...
# Modified to run on Python 3 and Raspian Desktop (so no serial interface is defined)
...
def play_sound(path):
  ...
  # Get sounds from /sounds sub-directory
  fullPath = os.path.join("sounds", path)
  sound = _sound_library.get(fullPath)
  #sound = _sound_library.get(path)
  if sound == None:
    canonicalized_path = fullPath.replace('/', os.sep).replace('\\', os.sep)
...
class Button:
	...
	def __init__(self, rect, **kwargs):
	  ...
	  self.sound    = None # Name of sound file to play
	  #for key, value in kwargs.iteritems():        #Python 2
	  for key, value in kwargs.items():
	    ...
	    elif key == 'sound': self.sound    = value
...
buttons = [
  ...
  [Button(( 30,  0,320, 60), bg='box'),
   Button(( 30, 60, 60, 60), bg='1',     cb=numericCallback, value=1, sound='Dtmf-1.wav'),
   Button(( 90, 60, 60, 60), bg='2',     cb=numericCallback, value=2, sound='Dtmf-2.wav'),
   Button((150, 60, 60, 60), bg='3',     cb=numericCallback, value=3, sound='Dtmf-3.wav'),
   Button(( 30,110, 60, 60), bg='4',     cb=numericCallback, value=4, sound='Dtmf-4.wav'),
   Button(( 90,110, 60, 60), bg='5',     cb=numericCallback, value=5, sound='Dtmf-5.wav'),
   Button((150,110, 60, 60), bg='6',     cb=numericCallback, value=6, sound='Dtmf-6.wav'),
   Button(( 30,160, 60, 60), bg='7',     cb=numericCallback, value=7, sound='Dtmf-7.wav'),
   Button(( 90,160, 60, 60), bg='8',     cb=numericCallback, value=8, sound='Dtmf-8.wav'),
   Button((150,160, 60, 60), bg='9',     cb=numericCallback, value=9, sound='Dtmf-9.wav'),
   Button(( 30,210, 60, 60), bg='star',  cb=numericCallback, value=0, sound='Dtmf-star.wav'),
   Button(( 90,210, 60, 60), bg='0',     cb=numericCallback, value=0, sound='Dtmf-0.wav'),
   Button((150,210, 60, 60), bg='hash',  cb=numericCallback, value=0, sound='Dtmf-pound.wav'),
   Button((180,260, 60, 60), bg='del2',  cb=numericCallback, value=10),
   Button(( 90,260, 60, 60), bg='call',    cb=numericCallback, value=12),
   Button(( 30,260, 60, 20), bg='cancel', cb=numericCallback, value=13)],
...
print ("Initting...")
#pygame.init()
#snd = pygame.mixer.Sound("sounds/Dtmf-0.wav")
#print ("Setting Mouse invisible...")
#pygame.mouse.set_visible(False)
print ("Setting fullscreen...")
#modes = pygame.display.list_modes(16)
modes = pygame.display.list_modes(0)    #TEST on Raspian Desktop
#screen = pygame.display.set_mode(modes[0], FULLSCREEN, 16)
screen = pygame.display.set_mode(modes[13], FULLSCREEN, 16)     #TEST on Raspian Desktop
...
print ("mainloop..")
while(True):
  # Check to exit application
  if threadExited == True:
    #print ("threadExited=" + str(threadExited))
    break
    ...
    for event in pygame.event.get():
      if(event.type is MOUSEBUTTONDOWN):
        ...
        for b in buttons[screenMode]:
          if b.selected(pos):
              if (b.sound != None) and (mute == False):
                  play_sound(b.sound)
              if b.value == 13:
                  threadExited = True  #Cancel button was pushed - signal exit
              break
